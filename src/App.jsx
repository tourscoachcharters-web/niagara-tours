...
import { 
  ...
  ShieldCheck, Coffee, Camera, Waves, Loader2
} from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(toursCollection, (snap) => {
      if (snap.empty) {
        ...
        seedData.forEach(t => {
          const docId = t.name.toLowerCase().replace(/\s+/g, '-');
          setDoc(doc(toursCollection, docId), t).catch(err => console.error("Seed error:", err));
        });
      } else {
      ...
      setErrorMsg("Sync error. Please refresh.");
    });
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                {tours.map(t => (
                  <TourCard 
                    key={t.id} 
                    tour={t} 
                    onSelect={(tour) => {setSelectedTour(tour); setView('booking');}} 
                    onDetail={(tour) => {setSelectedTour(tour); setView('detail');}} 
                  />
                ))}
              </div>
            </section>
